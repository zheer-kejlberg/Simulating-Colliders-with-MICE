...
DAG_xy_collider <- function(N, confounding = F) {
  ...
  #df <- as.data.frame(cbind(V2,V1,collider,u.c))
  return(df)
...
DAG_xz_collider <- function(N, confounding = F) {
  ...
  #df <- as.data.frame(cbind(V1,V2,z,collider,u.c))
  return(df)
}
...
DAG_M_bias <- function(N, confounding = F) {
  ...
  #df <- as.data.frame(cbind(V1,V2,z,z_2,collider,u.c))
  return(df)
  if(confounding) {print(1)}
}
...
simulation_missingness <- function(n_replicates, N, DAG, formula = "V2 ~ V1 + z", missing_var = "z", 
  ...
  dat <- dat[!names(dat) %in% c("u.V2", "u.z", "u.z_2")]
  ...
  # Get results
  outputs <- cbind(res.full[2,1],
                   res.ld[2,1],
                   res.mi[2,2],
                   #res.mi.no_uz[2,2],
                   res.mi.no_uc[2,2],
                   #res.mi.no_uc_or_uz[2,2],
                   res.mi.nocol[2,2],
                   #res.mi.nocol_nouz[2,2],
                   res.mi.nocol_nouc[2,2],
                   #res.mi.nocol_nouc_nouz[2,2],
                   res.full[2,4],
                   res.ld[2,4],
                   res.mi[2,6],
                   #res.mi.no_uz[2,6],
                   res.mi.no_uc[2,6],
                   #res.mi.no_uc_or_uz[2,6],
                   res.mi.nocol[2,6],
                   #res.mi.nocol_nouz[2,6],
                   res.mi.nocol_nouc[2,6]#,
                   #res.mi.nocol_nouc_nouz[2,6]
                   )
  return(outputs)
}
...
repeats <- function(n_replicates, N, input_DAG, formula, missing_var, missingness_mechanism, confounding = F) {
  ...
  if (confounding) {t1 <- paste0(t1, " [confounded]")}
  ...
  sims <- data.frame(foreach(i = 1:n_replicates, 
                            .combine = "rbind", 
                            .packages = c("mice", "stringr"),
                            .export = c("simulation_missingness", "summary_lm", "summary_lm_mice")) %dopar% {
                              simulation_missingness(n_replicates, N, input_DAG, formula, missing_var, missingness_mechanism, confounding)
                            })
  stopCluster(cl)
  
  sims_estimate <- sims[1:6]
  sims_pval <- sims[7:12]
  ...
  names(sims_estimate) <- names(sims_pval) <- c("Iter", "Full","LD","MI",
                                                "MI-no u.c","MI-no c", "MI-no c or u.c")
...
sim_print_values <- function(input) {
  ...
  pvals <- input$P_vals
  print("Estimates:")
  print(round(colMeans(ests[2:ncol(ests)]),3))
  print("False positive rates:")
  print(colMeans(pvals[2:ncol(pvals)]<.05))
}

# Function to remove superflucous columns from some of the simulations
...
t2.11 <- repeats(250, 1000, DAG_xz_collider, "V2~V1+z", "z", c("V1", "u.c"), TRUE)
t2.12 <- repeats(250, 1000, DAG_xz_collider, "V2~V1+z", "z", c("V1", "collider"), TRUE)
t2.13 <- repeats(250, 1000, DAG_xz_collider, "V2~V1+z", "z", c("V1", "u.z"), TRUE)

t2.14 <- repeats(250, 1000, DAG_xz_collider, "V1~V2+z", "z", c("u.z", "u.c"), TRUE)
t2.15 <- repeats(250, 1000, DAG_xz_collider, "V1~V2+z", "z", c("u.z", "collider"), TRUE)
t2.16 <- repeats(250, 1000, DAG_xz_collider, "V1~V2+z", "z", c("u.z"), TRUE)

t2.17 <- repeats(250, 1000, DAG_xz_collider, "V2~V1+z", "V2", c("V1", "u.c"), TRUE)
t2.18 <- repeats(250, 1000, DAG_xz_collider, "V2~V1+z", "V2", c("V1", "collider"), TRUE)
t2.19 <- repeats(250, 1000, DAG_xz_collider, "V2~V1+z", "V2", c("V1", "u.z"), TRUE)
...
t3.9 <- repeats(250, 1000, DAG_M_bias, "V1~V2+collider+z_2", "z", c("V2", "u.z"))
